        ...
        <tbody>
          {rows.map((row) => (
            <tr
              ...
              className={cn(
                "rounded-xl bg-secondary/40",
                isUs(row.team) && "bg-primary/15 ring-1 ring-primary/40",
              )}
            >
              <td className="rounded-l-xl px-3 py-3 text-muted-foreground">{row.position}</td>
              ...
              <td className="rounded-r-xl px-3 py-3 text-center font-bold">{row.points}</td>
            </tr>
          ))}
        </tbody>
      </table>
  ...
  return (
    <div
      className={cn(
        ...
        (isUs(match.home) || isUs(match.away)) && "bg-primary/12 ring-1 ring-primary/30",
      )}
    >
      <div className="min-w-0 flex-1 text-right">
        <TeamName team={match.home} />
      </div>
      <div className="shrink-0 rounded-lg bg-background/70 px-3 py-1 text-center font-semibold tabular-nums">
...
export function MatchMeta({ match }: { match: Match }) {
  return (
    <p className="mt-1 px-1 text-[11px] uppercase tracking-wider text-muted-foreground">
