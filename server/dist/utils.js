...
exports.parseZioneSchedule = parseZioneSchedule;
exports.parseZioneTeamSchedule = parseZioneTeamSchedule;
exports.parseZioneResults = parseZioneResults;
exports.parseZioneScheduleTeams = parseZioneScheduleTeams;
exports.parseZioneStandings = parseZioneStandings;
exports.buildUrl = buildUrl;
exports.fetchWithCookies = fetchWithCookies;
exports.tableToJson = tableToJson;
exports.parseTeamInfo = parseTeamInfo;
exports.parseSelects = parseSelects;
exports.extractHorarios = extractHorarios;
exports.parseCandidateIDsFromLinks = parseCandidateIDsFromLinks;
...
function fixEncodingArtifacts(text) {
    ...
    fixed = fixed.replace(/([cstmprldgnCSTMRLDGN])i�n/g, (_, letter) => `${letter}ión`);
 ...
 */
function parseSpanishDateRange(rangeStr) {
    if (!rangeStr)
 ...
 * Example: "Jornada 1" -> 1
 */
function extractJornadaNumber(label) {
    if (!label)
        return null;
 ...
 */
function parseDataRow($, $row, baseUrl) {
    ...
    const row = { Lugar: lugar, Equipo: equipo };
 ...
 */
function parseMatchRow($, $row, currentDate, baseUrl) {
    const cells = $row.find('td');
    if (cells.length < 9)
        return null; // Need at least 9 cells for a valid match
    // Extract time
 ...
 */
function parseRestTeams($, baseUrl) {
    const rest = [];
 ...
 */
function parseZioneSchedule($, baseUrl = 'https://consola.zione.com.mx') {
    // Extract metadata
    ...
    rows.each((_, row) => {
        ...
        const cells = $row.find('td');
        if (cells.length === 0)
            return; // Skip rows without cells
...
function extractGroupFromSubtitle(subtitle) {
    if (!subtitle)
        return null;
    const match = subtitle.match(/Grupo\s+([^/]+)/i);
    return match ? match[0].trim() : null;
}
function parseTeamScheduleRow($, $row, baseUrl, primaryTeam, fallbackDate, groupLabel) {
    const cells = $row.find('td');
    if (cells.length < 8)
        return null;
...
function parseZioneTeamSchedule($, options, baseUrl = 'https://consola.zione.com.mx') {
    const meta = extractScheduleMeta($);
    const headers = getScheduleHeaders();
    if (options.teamName) {
...
function extractStatusFromElement($, element, lookup) {
    ...
    for (const cls of classes) {
        if (lookup[cls])
            return lookup[cls];
        if (/^res_st_/i.test(cls))
            return cls;
    }
    return null;
}
function parseResultMatchRow($, $row, currentDate, baseUrl, statusLookup) {
    const cells = $row.find('td');
    if (cells.length < 9)
        return null;
    const timeText = $(cells[0]).text().trim();
    ...
    const status = extractStatusFromElement($, cells[6], statusLookup) ||
...
function parseZioneResults($, baseUrl = 'https://consola.zione.com.mx') {
    const meta = extractScheduleMeta($);
    ...
    const references = Object.entries(statusLookup).map(([className, label]) => ({ className, label }));
    return {
        type: 'results',
        meta,
        matchdays,
...
function parseZioneScheduleTeams($, baseUrl = 'https://consola.zione.com.mx') {
    let table = $('table.estadisticas');
    if (!table.length) {
        table = $('table').first();
    }
 ...
 */
function parseZioneStandings($, baseUrl = 'https://consola.zione.com.mx') {
    // Extract metadata
...
async function fetchWithCookies(url, options = {}, jar = { cookies: [] }) {
    ...
    try {
        ...
        const methodAny = (options.method || 'GET');
        ...
        const ensureEncoding = (encoding) => {
            ...
            return iconv_lite_1.default.encodingExists(normalized) ? normalized : 'utf-8';
        };
        const decodeBuffer = (encoding) => {
            try {
        ...
        const newJar = { cookies: [...jar.cookies] };
 ...
 */
function tableToJson($, tableElement, index) {
...
function parseTeamInfo($, teamId, baseUrl = 'https://consola.zione.com.mx') {
    ...
    if (!group) {
        const paragraphText = normalizeText(headerParagraph.clone().children('b').remove().end().text());
        group = cleanGroupValue(paragraphText);
    }
    const parseMatchSection = (titleContains) => {
        const matches = [];
...
function parseSelects($) {
    const selects = [];
    $('select').each((si, selectEl) => {
        const $select = $(selectEl);
        const options = [];
        $select.find('option').each((oi, optionEl) => {
 ...
 */
function parseCandidateIDsFromLinks($) {
    const gpoCandidates = [];
    let grupoSelectName = null;
